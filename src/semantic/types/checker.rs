...
//         let type_system_rc = Rc::new(RefCell::new(symbol_table_rc.borrow().type_system.clone()));
//
//         // TypeChecker {
//         //     symbol_table,
...
//         // }
//         TypeChecker {
//             symbol_table: symbol_table_rc,
//             type_system: type_system_rc,
//         }
//
//
//
...
//                 let symbol_id = self.symbol_table.borrow().lookup_symbol(name)?;
//
//                 // Récupérer le type associé au symbole
//                 if let Some(type_obj) = self.symbol_table.borrow().get_symbol_type(symbol_id)? {
//                     Ok(type_obj.id)
//                 } else {
...
//         match literal {
//             Literal::Integer { .. } => Ok(self.type_system.borrow().type_registry.type_int),
//             Literal::Float { .. } => Ok(self.type_system.borrow().type_registry.type_float),
//             Literal::Boolean(_) => Ok(self.type_system.borrow().type_registry.type_bool),
//             Literal::String(_) => Ok(self.type_system.borrow().type_registry.type_string),
//             Literal::Char(_) => Ok(self.type_system.borrow().type_registry.type_char),
...
//         let left_type = self.type_system.borrow().type_registry.get_type(left_type_id)
...
//             ))?;
//
//         let right_type = self.type_system.borrow().type_registry.get_type(right_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//                     Ok(self.type_system.borrow().type_registry.type_bool) // Résultat est toujours bool
//                 } else {
//                     Err(create_semantic_error(
//                         SemanticErrorType::TypeError(TypeError::TypeMismatch(
...
//                     Ok(self.type_system.borrow().type_registry.type_bool) // bool op bool -> bool
//                 } else {
//                     Err(create_semantic_error(
//                         SemanticErrorType::TypeError(TypeError::TypeMismatch(
...
//                     let range_type_id = self.type_system.borrow().type_registry.register_type(
...
//         let operand_type = self.type_system.borrow().type_registry.get_type(operand_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//                 match &operand_type.kind {
//                     TypeKind::Bool => Ok(self.type_system.borrow().type_registry.type_bool),
//                     _ => Err(create_semantic_error(
//                         SemanticErrorType::TypeError(TypeError::TypeMismatch(
...
//                 let ref_type_id = self.type_system.borrow().type_registry.create_reference_type(
//                     operand_type_id,
...
//         let func_type_clone;
//         {
//             let function_type = self.type_system.borrow().type_registry.get_type(function_type_id)
//                 .ok_or_else(|| create_semantic_error(
...
//             let arg_type = self.type_system.borrow().type_registry.get_type(arg_type_id)
//                 .ok_or_else(|| create_semantic_error(
...
//             let infer_type_var = self.type_system.borrow().create_type_variable(Some("ArrayElement".to_string()));
//             let infer_type_id = self.type_system.borrow().type_registry.register_type(TypeKind::Infer(infer_type_var));
//             return Ok(self.type_system.borrow().type_registry.create_array_type(infer_type_id, Some(0)));
//         }
//
//         // Vérifier le type du premier élément
//         let first_element_type_id = self.check_expression(&elements[0])?;
//         let first_element_type = self.type_system.borrow().type_registry.get_type(first_element_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//             let element_type = self.type_system.borrow().type_registry.get_type(element_type_id)
//                 .ok_or_else(|| create_semantic_error(
...
//         Ok(self.type_system.borrow().type_registry.create_array_type(first_element_type_id, Some(elements.len())))
//     }
//
...
//
//         let array_type = self.type_system.borrow().type_registry.get_type(array_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//             ))?;
//
//         let index_type = self.type_system.borrow().type_registry.get_type(index_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//         let object_type = self.type_system.borrow().type_registry.get_type(object_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//                 let member_type_var = self.type_system.borrow().create_type_variable(Some(format!("{}Member", member)));
//                 Ok(self.type_system.borrow().type_registry.register_type(TypeKind::Infer(member_type_var)))
//             },
//             _ => Err(create_semantic_error(
//                 SemanticErrorType::TypeError(TypeError::TypeMismatch(
...
//
//         let target_type = self.type_system.borrow().type_registry.get_type(target_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//             ))?;
//
//         let value_type = self.type_system.borrow().type_registry.get_type(value_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//         let method_return_var = self.type_system.borrow().create_type_variable(Some(format!("{}Return", method)));
//         Ok(self.type_system.borrow().type_registry.register_type(TypeKind::Infer(method_return_var)))
//     }
//
...
//         let target_type_id = self.type_system.borrow().type_registry.convert_ast_type(target_type);
//
//         let expr_type = self.type_system.borrow().type_registry.get_type(expr_type_id)
//             .ok_or_else(|| create_semantic_error(
...
//             ))?;
//
//         let target_type_obj = self.type_system.borrow().type_registry.get_type(target_type_id)
//             .ok_or_else(|| create_semantic_error(
//                 SemanticErrorType::TypeError(TypeError::TypeNotFound(format!("{:?}", target_type_id))),
//                 "Type not found".to_string(),
//                 Position { index: 0 }
...
//                 let declared_type_id = self.type_system.borrow().type_registry.convert_ast_type(ast_type);
...
//                     ))?;
//
//                 let declared_type = self.type_system.borrow().type_registry.get_type(declared_type_id)
//                     .ok_or_else(|| create_semantic_error(
...
//                 self.type_system.borrow().type_registry.convert_ast_type(ast_type)
//             },
//
//             (None, Some(expr)) => {
...
//             let param_type_id = self.type_system.borrow().type_registry.convert_ast_type(&param.parameter_type);
//             param_type_ids.push(param_type_id);
//         }
//
//         // Déterminer le type de retour
//         let return_type_id = match &func_decl.return_type {
//             Some(ast_type) => self.type_system.borrow().type_registry.convert_ast_type(ast_type),
//             None => self.type_system.borrow().type_registry.type_unit, // () par défaut
//         };
//
//         // Créer le type de la fonction
//         let function_type_id = self.type_system.borrow().type_registry.create_function_type(
//             param_type_ids,
//             return_type_id
//         );
//
